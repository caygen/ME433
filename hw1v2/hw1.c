...
int main() {
    ...
    TRISBbits.TRISB4 = 1; //choose rb4 to be input pin  (user)
    LATAbits.LATA4 = 1; 
   __builtin_enable_interrupts();
    ...
    while(1) {
        ...
        while(_CP0_GET_COUNT()<240000){;} //wait 1/1000sec
        LATAINV = 0x010;                 //invert ra4
        _CP0_SET_COUNT(0);               //reset core timer
        while(PORTBbits.RB4 == 0){        // wait if button pressed
        LATAbits.LATA4 = 0;        // clear the led while the button is pressed
        }                       
    }
