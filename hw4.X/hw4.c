...
#define CSdac LATBbits.LATB7// chip select pin
...
void initSPI1(){
    TRISBbits.TRISB7 = 0;       //set chip select pin to be output
    CSdac = 1;                  //CS active low
    SS1Rbits.SS1R = 0b0100;     //SS1 is RB7
    SDI1Rbits.SDI1R = 0b0000;   //SDI1 id RA1
    RPB8Rbits.RPB8R = 0b0011;   //SDO1 is RB8
    
    //setup spi1
    SPI1CON = 0;              // turn off the spi module and reset it
    SPI1BUF;                  // clear the rx buffer by reading from it
    SPI1BRG = 0x1;            // baud rate to 10 MHz [SPI1BRG = (48000000/(2*desired))-1] = 1.4 so pick the next int = 2
    SPI1STATbits.SPIROV = 0;  // clear the overflow bit
    SPI1CONbits.MODE32 = 0;   // use 8 bit mmode
...
void setVoltage(char channel, char voltage){
    CSdac = 0;                  // turn on spi connection
    unsigned char message = 0x00;
    if (channel == 1){          //if channel is B
        message |= 0xF0;
    }else if(channel == 0){     //if channel is A
        message |= 0x70;
    }
...
//        if(i>=199){i=0;}
      ...
      // DAC SPI: create sine and triangle wave  
      int i=0;
      for(i = 0; i < 1000; i++){
        _CP0_SET_COUNT(0);
        setVoltage(0,128-127*sin((2*PI*i*10)/1000));
        setVoltage(1,255*(i*5.0/1000));
